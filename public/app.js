...
const nf=new Intl.NumberFormat("ja-JP"), pct=v=>v==null?"―":`${(v*100).toFixed(1)}%`, signed=v=>v==null?"―":`${v>0?"+":""}${nf.format(Math.round(v))}`;
const diffClass=v=>v==null?"neutral":v>=0?"positive":"negative";
// diff/games are null when the source hasn't tallied a machine's result yet; never average those in as 0.
const validDiff=r=>r[3]!=null, validGames=r=>r[4]!=null;
const avgOf=(rows,idx,valid)=>{const v=rows.filter(valid);return v.length?v.reduce((s,r)=>s+r[idx],0)/v.length:null};
const winRateOf=rows=>{const v=rows.filter(validDiff);return v.length?v.filter(r=>r[3]>0).length/v.length:null};
...
function weekdayStats(rows){
  return weekdays.map((label,wd)=>{const a=rows.filter(r=>new Date(`${r[0]}T00:00:00Z`).getUTCDay()===wd);return{label,rows:a,count:a.filter(validDiff).length,days:new Set(a.map(r=>r[0])).size,avg:avgOf(a,3,validDiff),win:winRateOf(a),games:avgOf(a,4,validGames)}})
}
...
function renderMachineExplorer(name,rows){
  ...
  const nums=[...byNo].map(([no,a])=>({no,rows:a,avg:avgOf(a,3,validDiff),win:winRateOf(a),last:[...a].sort((x,y)=>y[0].localeCompare(x[0]))[0][0]})).sort((a,b)=>a.no-b.no);
  const ws=weekdayStats(rows), best=[...ws].filter(w=>w.days>=3&&w.avg!=null).sort((a,b)=>b.avg-a.avg)[0], gaps=spacingStats(rows);
  const avg=avgOf(rows,3,validDiff),win=winRateOf(rows),active=nums.filter(n=>n.last===summary.period.to).length;
  $("#machineExplorer").className="machine-explorer";
  $("#machineExplorer").innerHTML=`
    <div class="explorer-head"><div><p class="eyebrow">SELECTED MACHINE</p><h3>${esc(name)}</h3><p>${nums.length}台番で記録 / 最終日稼働 ${active}台 / ${new Set(rows.map(r=>r[0])).size}日</p></div><div class="headline-stat"><span>期間平均</span><strong class="${diffClass(avg)}">${signed(avg)}</strong><small>勝率 ${pct(win)}</small></div></div>
    ...
    <div class="subsection"><h4>曜日別の強さ</h4><div class="weekday-grid">${ws.map(w=>`<article><span>${w.label}</span><strong class="${diffClass(w.avg)}">${signed(w.avg)}</strong><small>${w.days}日 / ${w.count}台日<br>勝率 ${pct(w.win)}</small></article>`).join("")}</div></div>
    ...
    <div class="subsection"><h4>台番号をタップ</h4><div class="number-grid">${nums.map(n=>`<button class="number-chip ${n.last===summary.period.to?"current":"past"}" data-no="${n.no}"><b>${n.no}</b><span class="${diffClass(n.avg)}">${signed(n.avg)}</span><small>${n.last===summary.period.to?"最終日設置":"入替前"}</small></button>`).join("")}</div></div>
...
function renderMachineNumber(name,no,rows,machineRows){
  ...
  const sorted=[...rows].sort((a,b)=>b[0].localeCompare(a[0])), avg=avgOf(rows,3,validDiff),win=winRateOf(rows),games=avgOf(rows,4,validGames);
  const recent=sorted.slice(0,10),recentAvg=avgOf(recent,3,validDiff),ws=weekdayStats(rows),best=[...ws].filter(w=>w.count>=3&&w.avg!=null).sort((a,b)=>b.avg-a.avg)[0];
  ...
  const el=$("#machineNumberDetail");el.className="number-card";el.innerHTML=`<div class="number-title"><div><p class="eyebrow">NUMBER DETAIL</p><h3>${no}番台</h3><p>${esc(name)}として ${sorted.at(-1)[0]}〜${sorted[0][0]}</p></div></div><div class="number-stats"><span>平均差枚<strong class="${diffClass(avg)}">${signed(avg)}</strong></span><span>勝率<strong>${pct(win)}</strong></span><span>平均G<strong>${games==null?"―":nf.format(Math.round(games))}</strong></span><span>直近10件<strong class="${diffClass(recentAvg)}">${signed(recentAvg)}</strong></span></div><div class="detail-callout"><b>${best?`${best.label}曜日が最良`:"曜日判定不可"}</b><span>${best?`平均 ${signed(best.avg)}枚・${best.count}台日`:"サンプル不足"} / 高差枚の近接配置に ${spacedDays}日参加</span></div><div class="table-wrap"><table><thead><tr><th>日付</th><th>曜日</th><th>差枚</th><th>G数</th></tr></thead><tbody>${sorted.map(r=>`<tr><td>${r[0]}</td><td>${weekdays[new Date(`${r[0]}T00:00:00Z`).getUTCDay()]}</td><td class="${diffClass(r[3])}">${signed(r[3])}</td><td>${nf.format(r[4])}</td></tr>`).join("")}</tbody></table></div>`;el.scrollIntoView({behavior:"smooth",block:"nearest"});
}
...
async function loadNumber(){
  ...
  const avg=avgOf(rows,3,validDiff),win=winRateOf(rows);
  $("#numberResult").className="number-card";$("#numberResult").innerHTML=`<h3>${no}番台</h3><div class="number-stats"><span>平均差枚<strong class="${diffClass(avg)}">${signed(avg)}</strong></span><span>勝率<strong>${pct(win)}</strong></span><span>記録<strong>${rows.length}日</strong></span></div><div class="table-wrap"><table><thead><tr><th>日付</th><th>機種</th><th>差枚</th><th>G数</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${r[0]}</td><td class="machine">${esc(r[2])}</td><td class="${diffClass(r[3])}">${signed(r[3])}</td><td>${nf.format(r[4])}</td></tr>`).join("")}</tbody></table></div>`;
}
